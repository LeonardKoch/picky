...
var viewDataHelpers = function () {
    ...
    _createClass(viewDataHelpers, null, [{
        key: "createDayObject",
        value: function createDayObject(day, weekday, month, year, constraints, selections) {
        ...
        key: "isDateInDateList",
        value: function isDateInDateList(date, dateList) {
            return dateList.some(function (currentDate) {
                return _util2.default.aEqualToB(date, currentDate);
            });
        }
    }, {
        key: "generateMonthViewData",
        value: function generateMonthViewData(month, year, constraints, selections) {
            ...
            for (var i = 0; i < startWeekday; i++) {
                weeks[currentWeekIndex].push(viewDataHelpers.createDayObject(daysInPrecedingMonth - startWeekday + 1 + i, i, precedingMonth, yearOfPrecedingMonth, constraints, selections));
            }
            while (weeks[currentWeekIndex].length < 7 || currentDate <= daysInCurrentMonth) {
                ...
                if (currentDate <= daysInCurrentMonth) {
                    weeks[currentWeekIndex].push(viewDataHelpers.createDayObject(currentDate, weeks[currentWeekIndex].length + 1, month, year, constraints, selections));
                    currentDate++;
                } else {
                    weeks[currentWeekIndex].push(viewDataHelpers.createDayObject(currentDayInFollowingMonth, weeks[currentWeekIndex].length + 1, followingMonth, yearOfFollowingMonth, constraints, selections));
